...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "asu-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `asu-${platform}.zip` : `asu-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createAsuShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "asu.cmd"))) {
			writeFileSync(join(installDirectory, "asu.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\asu.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "asu.ps1"), '& "$PSScriptRoot/node_modules/.bin/asu.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "asu.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\asu.exe" %*\r\n');
		writeFileSync(join(installDirectory, "asu.ps1"), '& "$PSScriptRoot/node_modules/.bin/asu.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "asu"), join(installDirectory, "asu"));
}
...
if (!options.skipInstall) {
	...
	createAsuShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createAsuShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `asu-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "asu.exe" : "asu")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "asu.cmd" : "asu")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "asu.cmd" : "asu")} --help`);
	}
}
