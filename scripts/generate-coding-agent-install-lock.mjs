...
const installPackageName = "@hisn00w/asu-agent-install";
...
function createInstallerPackageJson(codingAgentPackage) {
	const packageJson = {
		...
		private: true,
		description: "Lockfile root used by the ASu Agent installer and updater.",
		dependencies: {
...
function validateGeneratedFiles(installerPackageJson, installLock, internalNames) {
	...
	const codingAgentPackageName = Object.keys(installerPackageJson.dependencies)[0];
	...
	for (const [lockPath, entry] of Object.entries(installLock.packages)) {
		...
		if (packageName === codingAgentPackageName && entry.version !== installerPackageJson.version) {
			errors.push(`${lockPath} coding agent version ${entry.version} does not match ${installerPackageJson.version}`);
		}
