...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before ASu Agent starts.
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/asu.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/asu"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating asu-$platform.zip..."
        (cd "$platform" && zip -r ../asu-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating asu-$platform.tar.gz..."
        mv "$platform" asu && tar -czf asu-$platform.tar.gz asu && mv asu "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../asu-$platform.zip)
    else
        tar -xzf asu-$platform.tar.gz && mv asu "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/asu.exe"
    else
        echo "  $OUTPUT_DIR/$platform/asu"
    fi
done
