...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".asu", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".asu", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".asu", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".asu"), { recursive: true });
			writeFileSync(join(cwd, ".asu", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".asu"), { recursive: true, force: true });
