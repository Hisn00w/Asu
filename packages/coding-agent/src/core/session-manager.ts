 ...
 * Encodes cwd into a safe directory name under ~/.asu/agent/sessions/.
 */
...
export class SessionManager {
		...
		if (existsSync(this.sessionFile)) {
			...
			if (this.fileEntries.length === 0) {
				...
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid ASu Agent session: ${explicitPath}`);
				}
				this.newSession();
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.asu/agent/sessions/<encoded-cwd>/).
	 */
