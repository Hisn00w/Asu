...
export class ScopedModelsSelectorComponent extends Container implements Focusable {
	...
	constructor(config: ModelsConfig, callbacks: ModelsCallbacks) {
		...
		this.addChild(
			new Text(theme.fg("muted", `仅对当前会话生效。按 ${keyText("app.models.save")} 保存到设置。`), 0, 0),
		);
		this.addChild(new Spacer(1));
	...
	private getFooterText(): string {
		...
		const parts = [
			...
			`${keyText("app.models.enableAll")} 全部启用`,
			`${keyText("app.models.clearAll")} 全部清除`,
			`${keyText("app.models.toggleProvider")} 提供商`,
			`${keyText("app.models.reorderUp")}/${keyText("app.models.reorderDown")} 调整顺序`,
