...
export class ModelSelectorComponent extends Container implements Focusable {
	...
	constructor(
		...
		if (scopedModels.length > 0) {
			...
			const hintText = "当前只显示已配置提供商的模型。使用 /login 添加提供商。";
	...
	private async refreshModels(): Promise<void> {
		...
		try {
			...
			if (result.aborted && timedOut) {
				this.errorMessage = "刷新模型超时，将显示缓存中的模型。";
			} else if (result.errors.size === 1) {
				this.errorMessage = `无法刷新 ${result.errors.keys().next().value}，将显示缓存中的模型。`;
			} else if (result.errors.size > 1) {
				this.errorMessage = `无法刷新 ${result.errors.size} 个模型目录（${[...result.errors.keys()].join(", ")}），将显示缓存中的模型。`;
			} else {
				this.errorMessage = this.modelRuntime.getError();
				if (!this.errorMessage) {
					this.refreshStatusMessage = "模型目录刷新完成。";
