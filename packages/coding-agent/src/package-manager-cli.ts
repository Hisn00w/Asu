...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|asu] [--self|--extensions|--models|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
更新 ASu Agent、已安装的包或模型目录。

Options:
  --self                  仅更新 ASu Agent（未指定目标时的默认行为）
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   更新 ASu Agent 和已安装的包
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 即使当前已是最新版本，也重新安装 ASu Agent

Short forms:
  ${APP_NAME} update                仅更新 ASu Agent
  ${APP_NAME} update --all          更新 ASu Agent 和全部扩展
  ${APP_NAME} update --models       仅刷新模型目录
  ${APP_NAME} update <source>       更新一个包
  ${APP_NAME} update asu            仅更新 ASu Agent（self 和 pi 也是别名）
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf = source === "self" || source === "asu" || source === "pi";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(
	...
	if (entrypoint) {
		console.error("");
		console.error(`ASu Agent 可执行文件位置：${entrypoint}`);
	}
}
