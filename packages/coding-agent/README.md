...
ASu Agent 是面向中文求职场景的终端智能助手，支持经历酥化、简历制作和秋招进度管理。本项目基于 [earendil-works/pi](https://github.com/earendil-works/pi) 开发，并保留其可扩展的终端 Agent 能力。
...
```bash
npm install -g --ignore-scripts @hisn00w/asu-agent
```
...
登录后直接使用 `asu`。可以输入 `/asu` 酥化经历、`/resume` 制作简历，或 `/offer` 管理求职进度；也可以直接说“我要酥化”。
...
**Custom providers & models:** Add providers via `~/.asu/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.asu/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
Sessions auto-save to `~/.asu/agent/sessions/` organized by working directory.
...
| Location | Scope |
|----------|-------|
| `~/.asu/agent/settings.json` | Global (all projects) |
| `.asu/settings.json` | Project (overrides global) |

See [docs/settings.md](docs/settings.md) for all options.

### Project Trust

On interactive startup, pi asks before trusting a project folder that contains project-local settings, resources, or project `.agents/skills` and has no saved decision for the folder or a parent folder in `~/.asu/agent/trust.json`. Trusting a project allows pi to load `.asu/settings.json` and `.asu` resources, install missing project packages, and execute project extensions.
...
If no extension or saved decision applies, `defaultProjectTrust` controls the fallback behavior. Set it to `"ask"`, `"always"`, or `"never"` in `~/.asu/agent/settings.json`, or change it with `/settings`.

`pi config` and package commands use the same project trust flow, except `pi update` never prompts. Pass `--approve` to trust project-local settings for one command or `--no-approve` to ignore them.

Use `/trust` in interactive mode to save a project trust decision for future sessions, including trust for the immediate parent folder. It writes `~/.asu/agent/trust.json` only; the current session is not reloaded, so restart pi for changes to take effect.
...
Replace the default system prompt with `.asu/SYSTEM.md` (project) or `~/.asu/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
```markdown
<!-- ~/.asu/agent/prompts/review.md -->
Review this code for bugs, security issues, and performance problems.
Focus on: {{focus}}
```

Place in `~/.asu/agent/prompts/`, `.asu/prompts/`, or a [pi package](#pi-packages) to share with others. See [docs/prompt-templates.md](docs/prompt-templates.md).
...
```markdown
<!-- ~/.asu/agent/skills/my-skill/SKILL.md -->
# My Skill
...
Place in `~/.asu/agent/skills/`, `~/.agents/skills/`, `.asu/skills/`, or `.agents/skills/` (from `cwd` up through parent directories) or a [pi package](#pi-packages) to share with others. See [docs/skills.md](docs/skills.md).
...
Place in `~/.asu/agent/extensions/`, `.asu/extensions/`, or a [pi package](#pi-packages) to share with others. See [docs/extensions.md](docs/extensions.md) and [examples/extensions/](examples/extensions/).

### Themes

Built-in: `dark`, `light`. Themes hot-reload: modify the active theme file and pi immediately applies changes.

Place in `~/.asu/agent/themes/`, `.asu/themes/`, or a [pi package](#pi-packages) to share with others. See [docs/themes.md](docs/themes.md).
...
| `PI_CODING_AGENT_DIR` | Override config directory (default: `~/.asu/agent`) |
