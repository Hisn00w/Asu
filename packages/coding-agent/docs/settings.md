...
| `.asu/settings.json` | Project (current directory) |
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.asu/agent/npm/`; project-scoped npm packages install under `.asu/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.
...
Paths in `~/.asu/agent/settings.json` resolve relative to `~/.asu/agent`. Paths in `.asu/settings.json` resolve relative to `.asu`. Absolute paths and `~` are supported.
...
Project settings (`.asu/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.asu/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .asu/settings.json (project)
{
