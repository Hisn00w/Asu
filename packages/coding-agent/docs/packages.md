...
By default, `install` and `remove` write to user settings (`~/.asu/agent/settings.json`). Use `-l` to write to project settings (`.asu/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup after the project is trusted.
...
- User installs go under `~/.asu/agent/npm/`.
- Project installs go under `.asu/npm/`.
...
- Cloned to `~/.asu/agent/git/<host>/<path>` (global) or `.asu/git/<host>/<path>` (project).
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. `pi config` starts in global settings (`~/.asu/agent/settings.json`); press Tab to switch between global and project-local modes. Use `pi config -l` to start in project overrides (`.asu/settings.json`) with inherited global resources dimmed.
